...
class DisasterEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.world = World(self.grid_size)
        self.rescue_agent = RescueAgent(start_pos=[0, 0])
        self.drone_agent = DroneAgent(start_pos=[9, 9])
        
        self.current_step = 0
        
        # Place agents on the grid
        self.world.grid[self.rescue_agent.pos[0]][self.rescue_agent.pos[1]] = self.rescue_agent.agent_id
        self.world.grid[self.drone_agent.pos[0]][self.drone_agent.pos[1]] = self.drone_agent.agent_id
        ...
        return self.world.memory_grid.copy(), {}
    ...
    def step(self, action):
        ...
        # Target Evaluation
        if self.rescue_agent.tile_under == 2:
            reward += 100 
            self.world.civilians_saved += 1
            self.rescue_agent.tile_under = 0 # Remove civilian from map
            
            if self.world.civilians_saved == self.world.civilians_to_save:
                reward += 200 
                done = True
        elif self.rescue_agent.tile_under == 1:
            reward -= 100 
        ...
        return self.world.memory_grid.copy(), reward, done, truncated, {}
